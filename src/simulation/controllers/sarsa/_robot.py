...
class MyRobot:
    def __init__(self, robot_instance: Robot) -> None:
        ...
        self.max_speed = 6.28
        self.turn_speed = 0.5
        self.right_motor_device = robot_instance.getDevice("right wheel motor")
        self.left_motor_device = robot_instance.getDevice("left wheel motor")
        ...
        self.right_speed: device.Device = 0
        self.left_speed: device.Device = 0
    ...
    def obstacle_detected(self) -> bool:
        self.dsValues = [self.dsValues[i].getValue() for i in range(len(self.dsNames))]
