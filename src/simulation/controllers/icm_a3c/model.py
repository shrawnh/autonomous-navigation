...
def categorical_sample(logits, d):
    value = tf.squeeze(
        tf.multinomial(logits - tf.reduce_max(logits, [1], keep_dims=True), 1), [1]
    )
...
class LSTMPolicy(object):
    ...
    def value(self, ob, c, h):
        sess = tf.get_default_session()
        return sess.run(
            self.vf, {self.x: [ob], self.state_in[0]: c, self.state_in[1]: h}
        )[0]
...
class StateActionPredictor(object):
    def __init__(self, ob_space, ac_space, designHead="universe"):
        ...
        f = linear(
            f, phi1.get_shape()[1].value, "flast", normalized_columns_initializer(0.01)
        )
        self.forwardloss = 0.5 * tf.reduce_mean(
            tf.square(tf.subtract(f, phi2)), name="forwardloss"
        )
    ...
    def pred_bonus(self, s1, s2, asample):
        ...
        error = sess.run(
            self.forwardloss, {self.s1: [s1], self.s2: [s2], self.asample: [asample]}
        )
...
class StatePredictor(object):
    ...
    def __init__(self, ob_space, ac_space, designHead="universe", unsupType="state"):
        ...
        f = tf.nn.relu(
            linear(
                f, phi1.get_shape()[1].value, "f1", normalized_columns_initializer(0.01)
            )
        )
