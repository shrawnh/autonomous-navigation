...
use_tf12_api = distutils.version.LooseVersion(
    tf.VERSION
) >= distutils.version.LooseVersion("0.12.0")
...
def process_rollout(rollout, gamma, lambda_=1.0, clip=False):
    ...
    if clip:
        rewards_plus_v[:-1] = np.clip(
            rewards_plus_v[:-1], -constants["REWARD_CLIP"], constants["REWARD_CLIP"]
        )
...
class RunnerThread(threading.Thread):
    ...
    def __init__(
        self, env, policy, num_local_steps, visualise, predictor, envWrap, noReward
    ):
        threading.Thread.__init__(self)
        self.queue = queue.Queue(
            5
        )  # ideally, should be 1. Mostly doesn't matter in our case.
...
def env_runner(
    env, policy, num_local_steps, summary_writer, render, predictor, envWrap, noReward
):
    """
    ...
    while True:
        ...
        for _ in range(num_local_steps):
            ...
            if terminal or length >= timestep_limit:
                ...
                if predictor is not None:
                    print(
                        "Episode finished. Sum of shaped rewards: %.2f. Length: %d. Bonus: %.4f."
                        % (rewards, length, life_bonus)
                    )
                    life_bonus = 0
                else:
                    print(
                        "Episode finished. Sum of shaped rewards: %.2f. Length: %d."
                        % (rewards, length)
                    )
                if "distance" in info:
            ...
            if info:
                ...
                if terminal:
                    summary.value.add(
                        tag="global/episode_value", simple_value=float(values)
                    )
                    values = 0
                    if predictor is not None:
                        summary.value.add(
                            tag="global/episode_bonus", simple_value=float(ep_bonus)
                        )
                        ep_bonus = 0
    ...
    def __init__(
        ...
        with tf.device(worker_device):
            ...
            if self.unsup:
                if "state" in unsupType:
                    self.predloss = (
                        constants["PREDICTION_LR_SCALE"] * predictor.forwardloss
                    )
                else:
            ...
            if use_tf12_api:
                ...
                if self.unsup:
                    ...
                    tf.summary.scalar(
                        "model/predgrad_global_norm", tf.global_norm(predgrads)
                    )
                    tf.summary.scalar(
            ...
            if self.unsup:
                sync_var_list += [
                    v1.assign(v2)
                    for v1, v2 in zip(predictor.var_list, self.ap_network.var_list)
                ]
